...
| What | Where |
| --- | --- |
| New theme | Folder under `apps/web/src/themes/<id>/` with `theme.json`, `theme.css`, `preview.png`, and either `mascots/<kind>.svg` files (svg mode) or `mascots/sprites.png` + `mascot_sprite` spec (sprite mode) |
