...
| `install-hooks.mjs` | Adds `owner: "agent-zoo"` entries to `~/.claude/settings.json` for all 13 consumed events. Atomic write. Refuses to overwrite an unparseable settings.json. Preserves symlinks and file mode. Also scrubs leftover entries from `settings.local.json` (which Claude Code ignores at user scope). |
| `uninstall-hooks.mjs` | Removes only entries with `owner: "agent-zoo"` (and the legacy `"claude-dashboard"` marker) from both `settings.json` and `settings.local.json`. Prunes empty arrays/keys. Atomic. |
